...
//	}
	...
	@Override
	public List<RechargeHistoryDTO> getAllRechargeHistoryOfStudent(Long id) {
		List<RechargeHistory> rechargeList = rechargeRepo.findByStudentStudentId(id);
		return rechargeList.stream().map(item -> mapper.map(item, RechargeHistoryDTO.class)).collect(Collectors.toList());
	}
