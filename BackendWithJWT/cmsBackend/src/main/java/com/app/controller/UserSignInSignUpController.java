...
//	private AdminService adminService;
	...
	// sign up
	@PostMapping("/signup")
	public ResponseEntity<?> userSignup(@RequestBody @Valid StudentDTO dto) {
		System.out.println("in sign up " + dto);
		System.out.println("here    ");
		return ResponseEntity.status(HttpStatus.CREATED).body(userService.registerNewUser(dto));
	}
	@PostMapping("/admin/signup")
	public ResponseEntity<?> adminSignup(@RequestBody @Valid Signup dto) {
		System.out.println("in sign up " + dto);
		System.out.println("here    ");
		return ResponseEntity.status(HttpStatus.CREATED).body(userService.registerNewAdmin(dto));
	}

	@PostMapping("/signin")
	public ResponseEntity<?> signinUser(@RequestBody @Valid SigninRequest reqDTO) {
		System.out.println("in signin " + reqDTO);
		System.out.println("before Authentication verifiedAuth = mgr\n"
				+ "				.authenticate(new UsernamePasswordAuthenticationToken");
		Authentication verifiedAuth = mgr
				.authenticate(new UsernamePasswordAuthenticationToken(reqDTO.getEmail(), reqDTO.getPassword()));
		System.out.println("after Authentication verifiedAuth = mgr\n"
				+ "				.authenticate(new UsernamePasswordAuthenticationToken");
